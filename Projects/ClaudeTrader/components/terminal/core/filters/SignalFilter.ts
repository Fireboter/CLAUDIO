...
export class SignalFilter {
    ...
    compute(
        ...
        if (config.usePivotConfirmation) {
            // Build a timestamp→index map so we can place the signal arrow on
            // the confirmation day's bar (where the trade actually triggers).
            // For cross-day confirmations the confirmation minute is on day D+1
            // while the early-pivot triangle lives on day D.  The arrow therefore
            // appears one bar to the right of the triangle, which is
            // trading-accurate: the entry fires when the recoil threshold is
            // crossed, not when the trendline touch first occurred.
            const dayTsToIdx = new Map<number, number>();
            for (let i = 0; i < days.length; i++) {
                dayTsToIdx.set(days[i].time, i);
            }

            for (const ep of confirmedEarlyPivots) {
                if (ep.status !== 'confirmed' || ep.confirmedAt === undefined) continue;

                // time = the actual minute the recoil threshold was crossed.
                const time  = ep.confirmMinuteTime ?? ep.time;
                const price = ep.confirmedAt;

                // In playback, suppress entry signals whose confirmation minute
                // hasn't been reached yet.
                if (playbackTime !== null && time > playbackTime) continue;
                const kind: SignalKind = ep.type === 'high' ? 'short' : 'long';

                // Place arrow on the bar that contains the confirmation minute.
                const confirmDayTs  = Math.floor(time / 86400) * 86400;
                const confirmDayIdx = dayTsToIdx.get(confirmDayTs) ?? ep.dayIndex;
