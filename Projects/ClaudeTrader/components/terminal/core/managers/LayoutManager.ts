...
import type { OrderBlockConfig } from '../models/OrderBlock';
...
export class LayoutManager extends Observable {
    ...
    // ─── Order block state (persisted here so OrderBlockManager can restore) ─
    orderBlockConfig: OrderBlockConfig = { ...DEFAULT_ORDER_BLOCK_CONFIG };
    ...
    private loadState(initialLoad = false): void {
        try {
            ...
            if (saved) {
                ...
                if (p.orderBlockConfig) this.orderBlockConfig = { ...DEFAULT_ORDER_BLOCK_CONFIG, ...p.orderBlockConfig };
            } else {
    ...
    private saveState(): void {
        try {
            localStorage.setItem(this.storageKey, JSON.stringify({
                ...
                orderBlockConfig:  this.orderBlockConfig,
            }));
    ...
    saveOrderBlockState(config: OrderBlockConfig): void {
        this.orderBlockConfig = config;
        if (typeof window !== 'undefined') this.saveState();
    }
