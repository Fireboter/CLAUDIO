...
export class PlaybackManager extends Observable {
    ...
    nextMinute(): void {
        ...
        if (dayIdx < 0) {
            ...
            const mins = days[dayIdx].minutes;
            // If playbackTime is not at an actual minute timestamp (e.g. at day midnight after
            // stopTimeline()), snap to the first market minute rather than skipping to index 1.
            const atActualMinute = mins.length > 0 && this.layout.playbackTime === mins[minIdx].time;
            if (!atActualMinute) {
                if (mins.length > 0) this._setMinute(dayIdx, 0);
            } else if (minIdx < mins.length - 1) {
