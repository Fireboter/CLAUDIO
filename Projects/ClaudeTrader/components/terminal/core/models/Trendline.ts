...
export interface TrendlineConfig {
    ...
    minAbsSlope:  number;   // 0–2 (min |slope| to show; 0 = off, hides near-horizontal lines)

    // NMS (Non-Maximum Suppression)
    useNMS:               boolean;
    nmsPriceTolerance:    number;   // 0–0.20
    nmsSlopeTolerance:    number;   // 0–1.0
    nmsLevelSlopeCutoff:  number;   // 0.001–0.20
    nmsLevelTolerance:    number;   // 0.0005–0.05
