...
export default function TimelineBar() {
    ...
    // Stable ref so the interval never needs to be recreated just because pauseTimeline
    // got a new function reference on a re-render.
    const pauseRef = useRef(pauseTimeline);
    useEffect(() => { pauseRef.current = pauseTimeline; }, [pauseTimeline]);

    // Auto-advance — uses PlaybackManager.nextMinute() so overnight gaps are skipped
    // and only actual market minutes are stepped through.
    useEffect(() => {
        if (!isPlaying || !dataTimeRange) return;

        // speed < 1x → slow the interval; speed >= 1x → more steps per 100 ms tick
        const TICK_MS = playbackSpeed < 1 ? Math.round(100 / playbackSpeed) : 100;
        const steps   = playbackSpeed >= 1 ? Math.round(playbackSpeed) : 1;

        intervalRef.current = setInterval(() => {
            const layout = terminalStore.layout;
            const pb     = terminalStore.playbackManager;
            for (let i = 0; i < steps; i++) {
                const before = layout.playbackTime;
                pb.nextMinute();
                if (layout.playbackTime === before) {
                    // nextMinute() made no progress — we're at the last minute
                    pauseRef.current();
                    return;
                }
            }
    ...
    // terminalStore is a module singleton — stable reference, intentionally omitted from deps
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [isPlaying, playbackSpeed, dataTimeRange]);
    ...
    const handlePlayPause = () => {
        if (isPlaying) { pauseTimeline(); }
            ...
            // Null means no cursor set yet; past endTime means previous run finished.
            // In both cases reset to the start of the analysis window so nextMinute()
            // begins at the first market minute of the boundary (or first) day.
            if (playbackTime === null || currentTime >= endTime) {
                setPlaybackTime(boundaryTime ?? startTime);
            }
            playTimeline();
        }
    };
    ...
    const handleSkip = (direction: 'forward' | 'backward') => {
        if (direction === 'forward') terminalStore.playbackManager.nextMinute();
        else terminalStore.playbackManager.prevMinute();
    };
