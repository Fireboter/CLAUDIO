...
export default function DataManagementPanel() {
    ...
    const handleDownload = async (symbol: string) => {
        try {
            await axios.post(`${API}/api/data/download`, { symbol });
        } catch (e) {
    ...
    const handleDelete = async (symbol: string) => {
        try {
            await axios.delete(`${API}/api/data/${symbol}`);
    ...
    return (
        <div className="space-y-4">
            {/* Refresh */}
            <button
                onClick={() => { setLoading(true); axios.get(`${API}/api/data/stats`).then(r => setInventory(r.data || [])).finally(() => setLoading(false)); }}
