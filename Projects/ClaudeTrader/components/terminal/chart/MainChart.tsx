...
export default function MainChart() {
    ...
    const {
        ...
        orderBlocks, provisionalOBTouches, confirmedOBTouches, orderBlockConfig,
        backtestSignals, hoveredBacktestEntry, tradeAxisConfig,
        signals, activeTrade, signalConfig,
    } = state;
    ...
    return (
        ...
        >
            ...
            {/* SVG order block overlay — horizontal pivot levels + touch zones */}
            <OrderBlockOverlay
                chart={chartApi}
                series={candleSeries}
                days={visibleDays}
                ...
                enabled={orderBlockConfig.enabled}
            />

            {/* SVG early pivot overlay — provisional markers, threshold lines, superseded */}
            {/* Also receives OB touch triangles so they share the same rendering path */}
            <EarlyPivotOverlay
                chart={chartApi}
                series={candleSeries}
                days={visibleDays}
                earlyPivots={[
                    ...earlyPivots,
                    ...earlyConfirmedPivots,
                    ...(orderBlockConfig.enabled ? [...provisionalOBTouches, ...confirmedOBTouches] : []),
                ].filter(p => p.dayIndex >= preHistoryCount)}
                config={earlyPivotConfig}
                enabled={earlyPivotConfig.enabled || orderBlockConfig.enabled}
            />

            {/* Live signal overlay — pivot confirmation + breakout entry/exit signals */}
            <SignalOverlay
                chart={chartApi}
                series={candleSeries}
                days={visibleDays}
                ...
                enabled={signalConfig.enabled && backtestSignals.length === 0}
                useStopLoss={tradeAxisConfig.useStopLoss}
                useTakeProfit={tradeAxisConfig.useTakeProfit}
            />

            {/* Backtest signal overlay — shown only after a backtest run (overrides live) */}
            <SignalOverlay
                chart={chartApi}
                series={candleSeries}
                days={visibleDays}
                ...
                activeTrade={null}
                enabled={backtestSignals.length > 0}
                useStopLoss={false}
                useTakeProfit={false}
                hoveredEntry={hoveredBacktestEntry}
            />
