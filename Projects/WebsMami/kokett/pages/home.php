<?php
$settings = [];
$rows = db_query('SELECT `key`, `value` FROM SiteSettings WHERE site = ?', [SITE_ID]);
...
// Fetch up to 5 product images per collection for slideshows (from ProductImage table)
$collectionIds = array_column($collections, 'id');
$collectionProductImages = [];
if ($collectionIds) {
    $placeholders = implode(',', array_fill(0, count($collectionIds), '?'));
    $piRows = db_query(
        "SELECT p.collectionId, pi.url FROM ProductImage pi
         JOIN Product p ON pi.productId = p.id
         WHERE p.collectionId IN ($placeholders)
           AND pi.url REGEXP 'p_[^/]+'
         ORDER BY p.id DESC, pi.displayOrder ASC",
        $collectionIds
    );
    foreach ($piRows as $row) {
        $cid = $row['collectionId'];
        if (!isset($collectionProductImages[$cid])) $collectionProductImages[$cid] = [];
        if (count($collectionProductImages[$cid]) >= 5) continue;
        if (!in_array($row['url'], $collectionProductImages[$cid]))
            $collectionProductImages[$cid][] = $row['url'];
    }
}
...
<!-- About Section -->
<section class="py-20" style="background:var(--c-bg)">
  ...
  function startCardRotators(grid) {
    grid.querySelectorAll('.col-card').forEach(function(card) {
      ...
      var t = setTimeout(function() {
        ...
        cardTimers.push(iv);
      }, cardIdx * Math.round(5000 / Math.max(total, 1)));
      cardTimers.push(t);
    });
  }
