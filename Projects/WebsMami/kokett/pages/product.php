...
if (empty($related)) {
    $related = db_query('SELECT id, name, price, discount, images FROM Product WHERE id != ? AND collectionId IS NOT NULL ORDER BY id DESC LIMIT 4', [$productId]);
}
...
<div class="container py-8" style="padding-left:2rem;padding-right:2rem">
  <div class="product-layout">
    ...
    <div>
      ...
      <div style="color:var(--color-gray-700);margin-bottom:1.5rem;line-height:1.75;font-size:0.95rem">
        <?php if (preg_match('/<[a-z][\s\S]*>/i', $product['description'])): ?>
          <?= $product['description'] ?>
        <?php else: ?>
  ...
  <!-- Collection Banner -->
  <?php if (!empty($product['collectionImage'])): ?>
  <div style="margin-top:4rem;border-radius:8px;overflow:hidden">
    <img src="<?= e($product['collectionImage']) ?>" alt="<?= e($product['collectionName'] ?? '') ?>"
