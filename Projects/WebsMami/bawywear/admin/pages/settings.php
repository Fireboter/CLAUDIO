<?php
$pageTitle = 'Ajustes';
try { $settingsRows = db_query('SELECT `key`, `value` FROM SiteSettings'); } catch (Throwable $_e) { $settingsRows = []; }
$s = [];
...
<form id="settings-form" class="admin-form" style="max-width:720px">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:2rem;align-items:start">
    <div>
      <?php
      $colorDefs = [
        'color_bg'             => ['Fondo',              'Página y header',                '#000000'],
        'color_surface'        => ['Superficie',         'Tarjetas, inputs, footer',       '#1a1a1a'],
        'color_primary'        => ['Primario',           'Botones, badges, acentos',       '#FFB800'],
        'color_text_on_primary'=> ['Texto sobre primario','Texto en botones con color primario','#000000'],
        'color_text'           => ['Texto',              'Cuerpo y navegación',            '#888888'],
        'color_border'         => ['Borde',              'Separadores y contornos',        '#333333'],
      ];
...
function updateOverlayPreview() {
  ...
  const sf = document.getElementById('cp-color_surface')?.value || '#1a1a1a';
